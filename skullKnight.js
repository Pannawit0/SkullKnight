...
function preload() {
	...
	let tilesGroup8 = new Tiles(['rrrrrrrr'], -210, 234, RailingTiles.w - 1, RailingTiles.h);
	...
	let tilesGroup9 = new Tiles(['h...h.....h...h'], -200, 135, Pillar2Tiles.w - 1, Pillar2Tiles.h);
	...
	let tilesGroup5 = new Tiles(['w..........................w'], -415, 175, Windows.w, Windows.h);
	...
	let tilesGroup6 = new Tiles(['d.................d'], -270, 175, Decors.w, Decors.h);
	...
	let tilesGroup7 = new Tiles(['t.................t'], -270, 165, Torches.w, Torches.h);
	...
	let tilesGroup3 = new Tiles(['====='], -450, 275, Floors.w, Floors.h);
	...
	LaserChargeAnim = loadAnimation('Images/Level/lasercharge.png', {
		frameSize: [64, 256],
		frames: 4,
		frameDelay: LaserSpeed
	});
	...
	TitleText1 = new Text.Sprite(225, 215);
	...
	MusicIntro.setVolume(0.15);
	MusicLoop.setVolume(0.15);
	MusicLoop.setLoop(true);
	MusicEnd.setVolume(0.15);
	...
	LaserSound.setVolume(0.25);

	DieSound = loadSound('Sounds/Die.wav');
	DieSound.setVolume(0.75);
...
function draw() {
	switch (scene) {
		case 0:
			...
			if (contro.presses('start') || kb.presses('enter') || kb.presses(' ') || mouse.presses()) {
				scene = 1;
		...
		case 1:
			...
			if (RoundPhrase == 1) {
				if (TextCheck != RoundPhrase) {
					...
					Round1Text.moveTowards(PlayerSprite.x, Round1Text.y, 0.2);
					LAButton.moveTowards(PlayerSprite.x, LAButton.y, 0.2);
				}
				...
				if (TextCheck != RoundPhrase) {
					...
					Round2Text.moveTowards(PlayerSprite.x, Round2Text.y, 0.2);
					XButton.moveTowards(PlayerSprite.x, XButton.y, 0.2);
				}
				...
				if (TextCheck != RoundPhrase) {
					...
					Round3Text.moveTowards(PlayerSprite.x, Round3Text.y, 0.2);
					R1Button.moveTowards(PlayerSprite.x + 10, R1Button.y, 0.2);
					RAButton.moveTowards(PlayerSprite.x - 10, R1Button.y, 0.2);
				}
				...
				if (TextCheck != RoundPhrase) {
					...
					Round4Text.moveTowards(PlayerSprite.x, Round4Text.y, 0.2);
					R2Button.moveTowards(PlayerSprite.x, R2Button.y, 0.2);
				}
			...
			// Skull Movement
			if (Dead == false) {
				Skull.moveTowards(PlayerSprite.x - 20, PlayerSprite.y - 25, 0.2);
			}
			...
			if (!Paused) {
				...
				if (Busy == false && Dead == false) {
					if ((-1 <= contro.leftStick.x && contro.leftStick.x <= -0.2) || kb.pressing('left')) {
						...
						PlayerSprite.vel.x =
							-PlayerSpeed * (-contro.leftStick.x || Math.min(0.83333333333, kb.pressing('left') / 30));
					} else if ((0.2 <= contro.leftStick.x && contro.leftStick.x <= 1) || kb.pressing('right')) {
						PlayerSprite.anis.scale.x = Scale;
						Skull.scale.x = Scale2;
						PlayerSprite.changeAni('run');
						...
						PlayerSprite.vel.x =
							PlayerSpeed * (contro.leftStick.x || Math.min(0.83333333333, kb.pressing('right') / 30));
					} else {
						Running = false;
						PlayerSprite.vel.x = 0;
						PlayerSprite.changeAni('idle');
					...
					if ((-1 <= contro.leftStick.x && contro.leftStick.x <= -0.2) || kb.pressing('left')) {
						...
						(0.2 <= contro.leftStick.x && contro.leftStick.x <= 1) ||
						(kb.pressing('right') && Dead == false)
					) {
						PlayerSprite.anis.scale.x = Scale;
						Skull.scale.x = Scale2;
				...
				// Roll
				if ((contro.presses('rt') || kb.presses('e')) && Busy == false && Dead == false) {
					Busy = true;
				...
				// Jump
				if ((contro.presses('a') || kb.presses('up') || kb.presses(' ')) && Busy == false && Dead == false) {
					Busy = true;
				...
				if (
					((contro.presses('r') &&
						(contro.rightStick.x > 0.2 ||
							contro.rightStick.y > 0.2 ||
							contro.rightStick.x < -0.2 ||
							contro.rightStick.y < -0.2)) ||
						mouse.presses()) &&
					Dead == false
					...
					let Aim = new Sprite(
						mouse.presses() ? mouse.x : PlayerSprite.x + contro.rightStick.x * 1000,
						mouse.presses() ? mouse.y : PlayerSprite.y + contro.rightStick.y * 1000,
						25,
						'n'
					);
					Aim.visible = false;
					...
					Fireball.moveTowards(Aim.x, Aim.y, mouse.presses() ? 0.05 : FireballSpeed);
					Fireball.rotateTo(Aim, 100, 0);
				}

				if (
					(contro.presses('b') || kb.presses('q')) &&
					Game_Start == false &&
					PlayerSprite.overlapping(StartGem) &&
					Dead == false
				) {
					// Game Start
					Game_Start = true;
				...
				if ((contro.presses('b') || kb.presses('q')) && PlayerSprite.overlapping(Gems) && Dead == false && !Paused) {
					for (let i = 0; i < Gems.length; i++) {
				...
				// Retry Button
				if (Dead == true && (contro.presses('b') || kb.presses('q') || kb.presses('enter'))) {
					Player_Retry();
				}
			}
			...
			if (Game_Start == true && !Paused) {
				TimerText.text = nf(round(FrameSinceStart / 60, 1), 0, 1) + 's';
			}

			// Pause input detect
			if (contro.presses('start') || kb.presses('enter')) {
				if (Paused == false) {
					world.autoStep = Paused;
					allSprites.autoUpdate = Paused;
					Paused = !Paused;
					PauseFrameStart.x = frameCount;
					MusicIntro.setVolume(0.02);
					MusicLoop.setVolume(0.02);
					PausedSound.play();
				} else {
					world.autoStep = Paused;
					allSprites.autoUpdate = Paused;
			...
			// Round Loops
			if (Game_Start == true && !Paused) {
				if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 240) {
					// Tutorial Laser
					RoundPhrase = 1;
					RoundDelay = 80;
					doLasers(5, PlayerSprite.x);
				} else if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 480 && FrameSinceStart >= 240) {
					// Tutorial Ice
					RoundPhrase = 2;
					RoundDelay = 120;
					doIce();
				} else if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 960 && FrameSinceStart >= 480) {
					// Tutorial Bat
					RoundPhrase = 3;
					RoundDelay = 240;
					doBatR();
				} else if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 1200 && FrameSinceStart >= 960) {
					// Tutorial Roll
					RoundPhrase = 4;
					RoundDelay = 1;
				} else if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 1440 && FrameSinceStart >= 1200) {
					// Roll Test
					RoundPhrase = 5;
					doLasers(6);
					RoundDelay = 120;
				} else if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 2700 && FrameSinceStart >= 1440) {
					// Real Round
					RoundPhrase = 6;
					RoundDelay = 200;
					doBatL();
					...
					doIce();
				} else if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 3000 && FrameSinceStart >= 2700) {
					// Break
					RoundDelay = 120;
				} else if (FrameSinceStart % RoundDelay == 0 && FrameSinceStart < 3300 && FrameSinceStart >= 3000) {
					// Super Laser Round
					RoundDelay = 120;
					doLasers(6);
				} else if (FrameSinceStart % RoundDelay == 0) {
					// Intense Round
					RoundDelay = 120;
					doBatL();
					doBatR();
